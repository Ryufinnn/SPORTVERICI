...
<div align=center><th>EDIT PEMBELIAN BARANG</th></div>
<hr>
<form method=post action="menu_gudang.php?module=editpembelian">
...
if($_POST[nofak]!=""){
	  ...
	  $query=mysql_query("select * from pembelian where no_order='$nofak'");
...
<form name="form1" method="post" action="">
  ...
  <table width="100%" border="0" cellspacing="6" cellpadding="6" bgcolor="#dedede">
    ...
    <tr>
	  ...
	  ?>
      <td align="left" valign="top"><input type="text" name="b5" id="b5" class="tb6" value="<?php echo"$sqljurusan[jml_pesan]" ; ?>">
...
FROM order_tmp LEFT JOIN barang ON order_tmp.kd_barang = barang.kd_barang";
  ...
  ?>
    <tr>
      ...
      <td><a href="?module=editpembelian&act=hapus&nofak=<?php echo"$nofak"; ?>&id=<?php echo $row['kd_barang']; ?>" title="Klik Disini Untuk Hapus Data "><img src='./Images/delete.png'></a></td>
...
if(! $_POST['nofak']=="")
{
$sqlUbah   = "select * from pembelian where no_order='$nofak'";
...
$sqlpelanggan 	= "select * from suplier  order by kd_suplier ASC";
$querypelanggan	= mysql_query($sqlpelanggan);
while($data1    = mysql_fetch_array($querypelanggan))
{
if($b6=="")
{
echo "<option value=0 selected>-Pilih Kode Suplier-</option>";
}
elseif ($b6==$data1[kd_suplier])
{
echo "<option value= $data1[kd_suplier] selected > $data1[kd_suplier] $data1[nm_suplier]</option>";
}
else
{
echo "<option value=  $data1[kd_suplier]> $data1[kd_suplier] $data1[nm_suplier]</option>";
}
}	
?>
</select>
    ...
    <tr>
      ...
      <td>
	   <?
	 $query3=mysql_query("select pel.*,p.* from pembelian p,suplier pel where pel.kd_suplier=p.kd_suplier and no_order='$nofak'");
	 $r7=mysql_fetch_array($query3);
	
	 ?>
	 
	  <input type="text" name="b4" id="b4" class="tb6" value="<?php echo"$r7[tgl]"; ?>">
	  
      <img src="./images/b_calendar.png" style="cursor: pointer;" onclick="javascript:NewCssCal('b4','yyyyMMdd')" /></td>
      <td>&nbsp;</td>
	  <tr>
	  <td><a href="menu_gudang.php?module=editpembelian'"></a>
        <input type="submit" name="button2" class="button blue" id="button2" value="Edit Pembelian" target=_blank>
      </td>
	 	  <td><input type="submit" name="buttonhapus" class="button red" value="Hapus"></td>
...
if($_GET['act']=="hapus")
{
	$q="Delete from order_tmp where kd_barang='".$_GET[id]."'";
		...
		if($res)
		{
		echo "<meta http-equiv='refresh' content='0; url=menu_gudang.php?module=editpembelian&nofak=$_GET[nofak]'>";
		}
}
?>
...
if($_POST[button])
{
	...
	if($trow==0)
	{
		...
		if($res)
		{
		echo "<meta http-equiv='refresh' content='0; url=menu_gudang.php?module=editpembelian&nofak=$nofak'>";
		}
	}else{
...
if($_POST[button3])
{
	$q="Delete from order_tmp";
		...
		if($res)
		{
		echo "<meta http-equiv='refresh' content='0; url=menu_gudang.php?module=editpembelian'>";
		}
}
?>
	...
	if($_POST[b4]=="" || $_POST[b6]=="" )
	{
		...
		if($trow==0)
		{
			echo "<script>alert('Data Pembelian belum ada')</script>";
				...
				$delete=mysql_query("delete from pembelian where no_order='$_POST[b1]'");
	...
	if($_POST[b4]=="" || $_POST[b6]=="" )
	{
		...
		if($trow==0)
		{
			...
			while($rowt=mysql_fetch_array($rest))
			{
				...
				$re=mysql_query("select * from pembelian where no_order='$_POST[b1]'");
				while($dat=mysql_fetch_array($re)){
				
					$stok_lama=mysql_fetch_array(mysql_query("select * from barang where kd_barang='$dat[kd_barang]'"));
					$stok2=$stok_lama[stok] - $dat[jml];
					
				$update1=mysql_query("update barang set stok='$stok2' where kd_barang='$dat[kd_barang]'");
			...
			include "Config/Connect.php";
				...
				if($resdt)
			{
			echo "<meta http-equiv='refresh' content='0; url=menu_gudang.php?module=editpembelian&aksi=selesai-edit-pembelian&idbeli=$_POST[b1]'>";
			     }
...
<?php
break;
case"selesai-edit-pembelian":

include "Config/Connect.php";
	  $q="SELECT * FROM pembelian where no_order='$_GET[idbeli]' ORDER BY no_order";
	  $res=mysql_query($q) or die(mysql_error());
	  	  while($r=mysql_fetch_array($res)){ 		
 echo"<p><img src='./images/cetak.png' height=15><a href='cetak-pembelian.php?idbeli=$r[no_order]' target='_blank' /><font color=red>Cetak</font></a></tr>";
echo "<h3>Detail Pembelian</h3><hr>";
  ...
  <table width="100%" border="1" cellspacing="6" cellpadding="6" bgcolor="#dedede"id='table-a' >
	<tr>
		...
		<td width="10%" align="left" valign="top">No.Order<br/><br/>Tanggal Faktur<br></td>
