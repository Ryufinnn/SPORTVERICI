...
<div align=center><th>EDIT PENJUALAN BARANG</th></div>
<hr>
...
WHERE jual_tmp.kd_barang = barang.kd_barang";
  ...
  ?>
    ...
    <tr>
      ...
      <td>
	   <?
	 $query3=mysql_query("select pel.*,p.* from penjualan_sales p,sales pel where pel.kd_sales=p.kd_sales and no_faktur='$nofak'");
	 ...
	 ?>
      ...
      <td>&nbsp;</td>
	  <tr>
	  <td><a href="menu_penjualan.php?module=editpenjualan'"></a>
	    <input type="submit" name="button2" class="button blue" id="button2" value="Edit Penjualan" target=_blank /></td> 
...
if($_POST[button])
{
	...
	if($trow==0)
	{
		...
		if($res)
		{
		echo "<meta http-equiv='refresh' content='0; url=menu_penjualan.php?module=editpenjualan&nofak=$nofak'>";
		}
	}else{
	...
	if($_POST[b4]=="" || $_POST[b6]=="" )
	{
		...
		if($trow==0)
		{
			...
			while($rowt=mysql_fetch_array($rest))
			{
				...
				$re=mysql_query("select * from penjualan_sales where no_faktur='$_POST[b1]'");
