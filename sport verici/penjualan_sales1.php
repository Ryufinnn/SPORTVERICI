...
<form name="form1" method="post" action="">
  ...
  <table width="100%" border="0" cellspacing="6" cellpadding="6" bgcolor="#dedede">
    <tr>
      <td width="13%" align="left" valign="top">No. Faktur</td>
      <td width="28%" align="left" valign="top"><label for="b8"></label>
	  
     <input type="text" name="b1" id="b1" class="tb6" value="<?php include "./generate_kode.php"; echo get_new_id("no_faktur","penjualan_sales","P-"); ?>"></td>
...
FROM jual_tmp LEFT JOIN barang ON jual_tmp.kd_barang = barang.kd_barang";
  ...
  ?>
    <tr>
      ...
      <td><a href="?module=jual_sales&act=hapus&id=<?php echo $row['kd_barang']; ?>" title="Klik Disini Untuk Hapus Data "><img src='./Images/delete.png'></a></td>
  ...
  ?>
    ...
    <tr>
     ...
     <td align="left" valign="top">&nbsp;<select name="b6" id="b6" class="tb6" onchange="submit()">
	  ...
	  if(isset($_POST[b6]))
	  {
		  $q="Select * from pelanggan where kd_pelanggan='".$_POST[b6]."'";
		  $res=mysql_query($q) or die(mysql_error());
		  $row=mysql_fetch_array($res);
		  echo "<option value='$row[kd_pelanggan]'>$row[kd_pelanggan] - $row[nm_pelanggan]</option>";		  
	  } else if(isset($_GET[id]))
	  {
		  $q="Select * from pelanggan where kd_pelanggan='".$_GET[id]."'";
		  $res=mysql_query($q) or die(mysql_error());
		  $row=mysql_fetch_array($res);
		  echo "<option value='$row[kd_pelanggan]'>$row[kd_pelanggan] - $row[nm_pelanggan]</option>";		  
	  }
	  ...
	  {
	  ?>
        <option value="<?php echo $row['kd_pelanggan']; ?>"><?php echo"$row[kd_pelanggan] - $row[nm_pelanggan]"; ?></option>
        <?php
	  }
		?>
      </select></td>
	...
	<tr>
      ...
      <td>Nama Sales</td>
     <td align="left" valign="top">&nbsp;<select name="ns" id="ns" class="ns" onchange="submit()">
      <?php
	  include "./Config/Connect.php";
	  if(isset($_POST[ns]))
	  {
		  $q1="Select * from sales where kd_sales='".$_POST[ns]."'";
		  $res1=mysql_query($q1) or die(mysql_error());
		  $row1=mysql_fetch_array($res1);
		  echo "<option value='$row1[kd_sales]'>$row1[kd_sales] - $row1[nm_sales]</option>";		  
	  } else if(isset($_GET[id1]))
	  {
		  $q1="Select * from sales where kd_sales='".$_GET[id1]."'";
		  $res1=mysql_query($q1) or die(mysql_error());
		  $row1=mysql_fetch_array($res1);
		  echo "<option value='$row1[kd_sales]'>$row1[kd_sales] - $row1[nm_sales]</option>";		  
	  }
	  ...
	  {
	  ?>
        <option value="<?php echo $row1['kd_sales']; ?>"><?php echo"$row1[kd_sales] - $row1[nm_sales]"; ?></option>
        <?php
	  }
		?>
      </select></td>
    ...
    <tr>
	  ...
	  <td><a href="menu_penjualan.php?module=jual_sales'"></a>
        <input type="submit" name="button2" class="button blue" id="button2" value="Simpan Penjualan" target=_blank>
      </td>
	  <td>&nbsp;</td>
	...
	if($_POST[b4]=="" || $_POST[b6]=="" )
	{
		...
		if($trow==0)
		{
			...
			include "Config/Connect.php";
				...
				if($resdt)
			{
			echo "<meta http-equiv='refresh' content='0; url=menu_penjualan.php?module=jual_sales&aksi=selesai-penjualan&idjual=$_POST[b1]'>";
			     }
